"""Run creation and explicit WatchGuard observation routes."""
...
from platform_api.schemas import (
    CreateRunRequest,
    ExecutePhishingEmailObservationRequest,
    ExecuteWatchGuardObservationRequest,
...
from platform_backends.phishing_email import (
    ...
    execute_predefined_observation as execute_phishing_email_predefined_observation,
)
...
def execute_watchguard_normalize_endpoint(
    run_id: str,
    request: ExecuteWatchGuardObservationRequest = Body(default_factory=ExecuteWatchGuardObservationRequest),
    runtime: AppRuntime = Depends(get_runtime),
) -> object:
    run = runtime.get_run_or_raise(run_id)
    ...
    if outcome.observation_result.status == ObservationStatus.FAILED:
        response_body["error"] = {
            ...
            "message": outcome.observation_result.errors[0],
        }
...
def execute_phishing_email_basic_assessment_endpoint(
    run_id: str,
    request: ExecutePhishingEmailObservationRequest = Body(default_factory=ExecutePhishingEmailObservationRequest),
    runtime: AppRuntime = Depends(get_runtime),
) -> object:
    run = runtime.get_run_or_raise(run_id)
    case = runtime.get_case_or_raise_from_run(run)
    ...
    outcome = execute_phishing_email_predefined_observation(
        run=run,
        input_artifact=input_artifact,
        input_payload=input_payload,
        observation_request=observation_request,
